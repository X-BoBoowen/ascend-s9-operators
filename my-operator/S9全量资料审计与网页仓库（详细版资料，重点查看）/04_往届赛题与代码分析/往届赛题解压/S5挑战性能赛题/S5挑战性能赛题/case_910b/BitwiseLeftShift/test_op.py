...
case_data = {
    'case1': {
        'input':np.random.uniform(-10, 10, [1024]).astype(np.int8),
        'other':np.random.uniform(0, 4, [1024]).astype(np.int8)
    }
}
...
class TestCustomOP(TestCase):
    def test_custom_op_case(self,num):
        ...
        output = torch.bitwise_left_shift(input_x, input_other)
        # 修改输入
        output_npu = custom_ops_lib.custom_op(input_x.npu(), input_other.npu(), int(num))

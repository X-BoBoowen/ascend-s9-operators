...
case_data = {
    'case1': {
        'input':torch.rand((128,1), dtype=torch.float16) * 2 - 1,
        'size':torch.tensor([128, 256], dtype=torch.int64)
    }
}
...
class TestCustomOP(TestCase):
    def test_custom_op_case(self,num):
        ...
        size = case_data[caseNmae]["size"]
        tensor_list = size.tolist()
        output = input_x.expand(tensor_list)
        # 修改输入
        size_npu = tuple(size.npu().tolist())
        output_npu = custom_ops_lib.custom_op(input_x.npu(), size_npu, int(num))

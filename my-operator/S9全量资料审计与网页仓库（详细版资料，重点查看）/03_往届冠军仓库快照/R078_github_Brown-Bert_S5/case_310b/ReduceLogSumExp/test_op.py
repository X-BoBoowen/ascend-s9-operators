...
case_data = {
    'case1': {
        'input':np.random.uniform(-50, 50, [16, 16, 32]).astype(np.float16),
        'axes':np.random.uniform(1, 1, [1]).astype(np.int32),
        'keepdim':False
    }
}
...
class TestCustomOP(TestCase):
    def test_custom_op_case(self,num):
        ...
        axes = torch.from_numpy(case_data[caseNmae]["axes"])
        keepdim = case_data[caseNmae]["keepdim"]
        output = torch.logsumexp(input_x, dim=axes.item(), keepdim=keepdim)
        # 修改输入
        print("123")
        output_npu = custom_ops_lib.custom_op(input_x.npu(), axes.npu(), keepdim, axes.item(), int(num))
        print("456")

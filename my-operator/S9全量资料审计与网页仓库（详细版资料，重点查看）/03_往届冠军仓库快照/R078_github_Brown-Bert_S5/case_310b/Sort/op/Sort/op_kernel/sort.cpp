...
public:
    ...
    // 快速排序的分区函数
    __aicore__ inilne int partition(int low, int high) {
        int pivot = arr[high]; // 选择最后一个元素作为基准
        int i = low - 1;       // i 是小于基准的区域的右边界
        half value = static_cast<DTYPE_Y1>(inputGm(low));
        for (int j = low; j < high; j++) {
            value = static_cast<DTYPE_Y1>(inputGm(j));
            if (value < pivot) {          // 如果当前元素小于基准
                i++;
                std::swap(arr[i], arr[j]); // 将小于基准的元素放到左侧
            }
        }

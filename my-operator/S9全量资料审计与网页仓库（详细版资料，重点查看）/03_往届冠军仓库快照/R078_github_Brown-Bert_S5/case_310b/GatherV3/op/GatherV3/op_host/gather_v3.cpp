...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
 ...
 //   auto dt1 = context->GetInputTensor(1)->GetDataType();
  ...
  auto batchDims = context->GetAttrs()->GetInt(0);

  bool bl = *(context->GetAttrs()->GetBool(1));
 ...
 //   std::cout << "ubsize = " << ubSize << std::endl;
  ...
  uint32_t dataSize = 1;
  for (int i = 0; i < dimension_indices; ++i){
    dataSize *= context->GetInputShape(1)->GetStorageShape().GetDim(i);
  }
...
//   tiling.set_dim(dim);
  ...
  tiling.set_batchDims(*batchDims);
...
static ge::graphStatus InferShape(gert::InferShapeContext* context)
{
    ...
    auto dimension_indices = context->GetInputShape(1)->GetDimNum();
    uint32_t dataSize = 1;
    for (int i = 0; i < dimension_indices; ++i){
      dataSize *= context->GetInputShape(1)->GetDim(i);
    }
    if (x1_shape->GetDimNum() == 4){
      y_shape->SetDimNum(2);
      y_shape->SetDim(0, x1_shape->GetDim(0) * x1_shape->GetDim(1) * dataSize);
      y_shape->SetDim(0, x1_shape->GetDim(3));
    }else{
      *y_shape = *x1_shape;
    }

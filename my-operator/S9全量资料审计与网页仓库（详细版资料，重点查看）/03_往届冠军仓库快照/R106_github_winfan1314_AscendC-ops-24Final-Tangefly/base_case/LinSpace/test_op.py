...
case_data = {
    'case1': {
        'start':np.array([-4096]).astype(np.float32),
        'stop':np.array([4096]).astype(np.float32),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case2': {
        'start':np.array([-1]).astype(np.int32),
        'stop':np.array([1]).astype(np.int32),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case3': {
        'start':np.array([-4096]).astype(np.int16),
        'stop':np.array([4096]).astype(np.int16),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case4': {
        'start':np.array([0]).astype(np.uint8),
        'stop':np.array([255]).astype(np.uint8),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case5': {
        'start':np.array([-4096]).astype(np.float16),
        'stop':np.array([4096]).astype(np.float16),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case6': {
        'start':torch.tensor([-4096], dtype=torch.bfloat16),
        'stop':torch.tensor([4096], dtype=torch.bfloat16),
        'num':np.array([2097152]).astype(np.int32)
    },
    'case7': {
        'start':np.array([-128]).astype(np.int8),
        'stop':np.array([127]).astype(np.int8),
        'num':np.array([2097152]).astype(np.int32)
    }
}
...
def verify(real_result, golden):
    ...
    print("Count: ", torch.sum(result > 0))
    ...
    if not result_rtol.all() and not result_atol.all():
        if torch.sum(result_rtol == False) > real_result.element_size() * loss and torch.sum(result_atol == False) > real_result.element_size() * loss: 
            print("[ERROR] result error")
            return False
    print("test pass")
    return True
...
class TestCustomOP(TestCase):
    def test_custom_op_case(self,num):
        ...
        if int(num) == 6:
            input_start = case_data[caseNmae]["start"]
            input_stop = case_data[caseNmae]["stop"]
            input_num = torch.from_numpy(case_data[caseNmae]["num"])
            golden = torch.linspace(input_start[0], input_stop[0], input_num[0]).to(torch.bfloat16)

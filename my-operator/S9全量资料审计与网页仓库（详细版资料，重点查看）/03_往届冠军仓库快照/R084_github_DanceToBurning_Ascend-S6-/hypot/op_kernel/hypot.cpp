...
public:
    ...
    __aicore__ inline void Process(int offsetA, int offsetB, int offsetC, int asize, int bsize)
    {
        ...
        if (asize == bsize) {
            ...
            float value;
            if(std::is_same_v<T,bfloat16_t>)value=ToFloat(valA);
            else value=(float)valA;
            if (value == 0.0f) {
                for (int32_t i = 0; i < loopCount; i++) {
                    int cur = min(len - i * chunksize, chunksize);
                    if (cur <= 0) continue;
            ...
            float value;
            if(std::is_same_v<T,bfloat16_t>)value=ToFloat(valB);
            else value=(float)valB;
            if (value == 0.0f) {
                for (int32_t i = 0; i < loopCount; i++) {
                    int cur = min(len - i * chunksize, chunksize);
                    if (cur <= 0) continue;
...
private:
    ...
    __aicore__ inline void Compute(int cur)
    {
        if constexpr (std::is_same_v<T, float>)
        {
            ...
            Mul(x1Local, x1Local, x1Local, cur);
            Mul(x2Local, x2Local, x2Local, cur);
            Add(x1Local, x1Local, x2Local, cur);
            Sqrt(yLocal, x1Local, cur);
        ...
        else 
        {
            ...
            Mul(floatBuf1,floatBuf1,floatBuf1,align_cur);
            Mul(floatBuf2,floatBuf2,floatBuf2,align_cur);
            Add(floatBuf1,floatBuf1,floatBuf2,align_cur);
            Sqrt(floatBuf3,floatBuf1,align_cur);
    ...
    __aicore__ inline void ComputeA(int cur, T val)
    {
        ...
        if constexpr(std::is_same_v<T,float>){
            ...
            float val_sq = (float)val * (float)val;
            Mul(x1Local, x1Local, x1Local, cur);
            Adds(x1Local, x1Local, val_sq, cur);
            Sqrt(yLocal, x1Local, cur);
        } else { 
            auto floatBuf1 = tmpBuffer1.Get<float>(); 
            auto floatBuf2 = tmpBuffer2.Get<float>(); 
            ...
            if constexpr(std::is_same_v<T, bfloat16_t>) {
                scalar_val = ToFloat(val);
            } else { 
                scalar_val = (float)val;
            }
            float scalar_sq = scalar_val * scalar_val;
            
            
            Mul(floatBuf1, floatBuf1, floatBuf1, align_cur);
            Adds(floatBuf1, floatBuf1, scalar_sq, align_cur);
            Sqrt(floatBuf2, floatBuf1, align_cur);
            
            constexpr auto rMode = std::is_floating_point_v<T> ? RoundMode::CAST_NONE : RoundMode::CAST_RINT;
            Cast(yLocal, floatBuf2, rMode, align_cur);
        }
    ...
    __aicore__ inline void ComputeB_Scalar(int cur, T val)
    {
        ...
        LocalTensor<T> x2Local = inQueueB.DeQue<T>();
        LocalTensor<T> yLocal = outQueueC.AllocTensor<T>();
        uint32_t align_cur = ((cur + blocklen - 1) / blocklen) * blocklen;
        ...
        if constexpr(std::is_same_v<T,float>){
            float val_sq = (float)val * (float)val;
            Mul(x2Local, x2Local, x2Local, cur);
            Adds(x2Local, x2Local, val_sq, cur);
            Sqrt(yLocal, x2Local, cur);
        } else {
            auto floatBuf1 = tmpBuffer1.Get<float>(); 
            auto floatBuf2 = tmpBuffer2.Get<float>();
            Cast(floatBuf1, x2Local, RoundMode::CAST_NONE, align_cur);
    ...
    {
        uint32_t align_cur = ((cur + blocklen - 1) / blocklen) * blocklen;
        auto bLocal = inQueueB.DeQue<T>();
        auto cLocal = outQueueC.AllocTensor<T>();

        if constexpr (std::is_same_v<T, float>) {
            Abs(cLocal, bLocal, cur);
        } else {
            auto floatBuf_in_out = tmpBuffer1.Get<float>();
            Cast(floatBuf_in_out, bLocal, RoundMode::CAST_NONE, align_cur);
            Abs(floatBuf_in_out, floatBuf_in_out, cur);

            constexpr auto rMode = std::is_floating_point_v<T> ? RoundMode::CAST_NONE : RoundMode::CAST_RINT;
            Cast(cLocal, floatBuf_in_out, rMode, align_cur);
        }
    ...
    __aicore__ inline void ComputeA_AbsX(int cur)
    {
        uint32_t align_cur = ((cur + blocklen - 1) / blocklen) * blocklen;
        auto x1Local = inQueueA.DeQue<T>();
        auto yLocal = outQueueC.AllocTensor<T>();
        ...
        if constexpr (std::is_same_v<T, float>) {
            Abs(yLocal, x1Local, cur); 
        } else {
            
            auto floatBuf_in_out = tmpBuffer1.Get<float>();
            Cast(floatBuf_in_out, x1Local, RoundMode::CAST_NONE, align_cur);
            Abs(floatBuf_in_out, floatBuf_in_out, cur); 
            
            constexpr auto rMode = std::is_floating_point_v<T> ? RoundMode::CAST_NONE : RoundMode::CAST_RINT;
            Cast(yLocal, floatBuf_in_out, rMode, align_cur);
        }
...
extern "C" __global__ __aicore__ void hypot(
    ...
    GM_ADDR workspace, GM_ADDR tiling)
{
 ...
 void hypot_do(uint32_t blockDim, void *l2ctrl, void *stream, uint8_t *x, uint8_t *y, uint8_t *z,
                    uint8_t *workspace, uint8_t *tiling)
 {
     hypot<<<blockDim, l2ctrl, stream>>>(x, y, z, workspace, tiling);
 }
 #endif
...
public:
    ...
    __aicore__ inline void InitStage1(GM_ADDR x, GM_ADDR y, GM_ADDR workspace, int32_t bigCoreNumA, int32_t bigCoreStrideA, int32_t bigCoreNumB, int32_t bigCoreStrideB, int32_t maxCalcNumStage1, int32_t mLength, int32_t nLength, int32_t kLength) {
        ...
        xGm.SetGlobalBuffer((__gm__ float*)x, mLength * kLength * 2);
        yGm.SetGlobalBuffer((__gm__ float*)y, kLength * nLength * 2);
    }
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR workspace, GM_ADDR bias, GM_ADDR z, int32_t mLength, int32_t nLength, int32_t kLength, int32_t bigCoreNumM, int32_t bigCoreStrideM, bool hasBias, int32_t biasNLength, int32_t biasMLength, TPipe *pipe){
        ...
        zGm.SetGlobalBuffer((__gm__ float*)z, mLength * nLength * 2);
        ...
        pipe->InitBuffer(outQueueZ, BUFFER_NUM_STAGE3, nLength * sizeof(float));
        ...
        if (hasBias){
            pipe->InitBuffer(inQueueBias, BUFFER_NUM_STAGE3, 2 * nLength * sizeof(float));
            biasGm.SetGlobalBuffer((__gm__ float*)bias, mLength * nLength * 2);
        }
    }
    ...
    __aicore__ inline void Compute(int32_t offset){
        ...
        if (hasBias){
            ...
            // if (offset == 0){
            //     DumpTensor(biasLocal, 97, 64);
            // }
            const uint32_t dest_dims[] = {(uint32_t)nLength * 2};
            const uint32_t src_dims[] = {(uint32_t)biasNLength * 2};
            BroadCast<float, 1, 0>(tmpFloatLocal1, biasLocal, dest_dims, src_dims);
            // if (offset == 0){
            //     DumpTensor(tmpFloatLocal1, 98, 64);
            //     DumpTensor(zLocal, 99, 64);
            // }
            Add(zLocal, zLocal, tmpFloatLocal1, nLength * 2);
            // if (offset == 0){
            //     DumpTensor(zLocal, 100, 64);
            // }
    ...
    __aicore__ inline void Process(TPipe *pipe){
        ...
        for (int32_t row = startIndexM; row < endIndexM; row++){
            int32_t offset = row * nLength;
            DumpTensor(aGm[offset], 379, 32);
            CopyIn(row);
            if (row == 0){
                printf("now coreid : %d \n", blockIndex);
                DumpTensor(aGm[offset + 32 * 5], 379, 64);
            }
            if (row == 17){
                printf("now coreid : %d \n", blockIndex);
                DumpTensor(aGm[offset + 32 * 5], 379, 64);

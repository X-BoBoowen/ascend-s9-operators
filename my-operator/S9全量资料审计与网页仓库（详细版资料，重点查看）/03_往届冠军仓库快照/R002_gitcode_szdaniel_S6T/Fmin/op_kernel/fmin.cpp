...
template <typename dataType, uint32_t tileKey> class KernelFmin;

template <typename T> class KernelFmin<T, 0> {
public:
    __aicore__ inline KernelFmin() {}

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, FminTilingData tiling)
    {
        ...
        if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            pipe.InitBuffer(tBuf[0], this->tileLength * sizeof(half));
            pipe.InitBuffer(tBuf[1], this->tileLength * sizeof(half));
            pipe.InitBuffer(tBuf[2], this->tileLength * sizeof(half));
        } else if constexpr (std::is_same_v<T, int64_t>) {
            pipe.InitBuffer(tBuf[0], this->tileLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[1], this->tileLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[2], this->tileLength * sizeof(int32_t));
    ...
    __aicore__ inline void Compute(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
                      std::is_same_v<T, int32_t>) {
            AscendC::Min(zLocal, xLocal, yLocal, len);
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            LocalTensor<half> tmpTensor1 = tBuf[0].Get<half>();
            LocalTensor<half> tmpTensor2 = tBuf[1].Get<half>();
            LocalTensor<half> tmpTensor3 = tBuf[2].Get<half>();
            ...
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, len);
            ...
            // LocalTensor<int32_t> tmpTensor1 = tBuf[0].Get<int32_t>();
            // LocalTensor<int32_t> tmpTensor2 = tBuf[1].Get<int32_t>();
            // LocalTensor<int32_t> tmpTensor3 = tBuf[2].Get<int32_t>();

            // AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, len);
            // AscendC::Cast(tmpTensor2, yLocal, AscendC::RoundMode::CAST_NONE, len);
            // AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, len);
            // AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_NONE, len);
        } else if constexpr (std::is_same_v<T, bfloat16_t>) {
...
template <typename T> class KernelFmin<T, 1> {
public:
    __aicore__ inline KernelFmin() {}
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, FminTilingData tiling)
    {
        ...
        pipe.InitBuffer(tBuf[1], this->tileLength * sizeof(T));
        if constexpr (std::is_same_v<T, bfloat16_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * sizeof(float));
            pipe.InitBuffer(tBuf[3], this->tileLength * sizeof(float));
            pipe.InitBuffer(tBuf[4], this->tileLength * sizeof(float));
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * sizeof(half));
            pipe.InitBuffer(tBuf[3], this->tileLength * sizeof(half));
            pipe.InitBuffer(tBuf[4], this->tileLength * sizeof(half));
        } else if constexpr (std::is_same_v<T, int64_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[3], this->tileLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[4], this->tileLength * sizeof(int32_t));
        }
    }
    ...
    __aicore__ inline void Compute1D(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
                      std::is_same_v<T, int32_t>) {
            AscendC::Min(zLocal, xLocal, broadcastTmpTensor, len);
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            LocalTensor<half> tmpTensor1 = tBuf[2].Get<half>();
            LocalTensor<half> tmpTensor2 = tBuf[3].Get<half>();
            LocalTensor<half> tmpTensor3 = tBuf[4].Get<half>();

            AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, len);
            AscendC::Cast(tmpTensor2, broadcastTmpTensor, AscendC::RoundMode::CAST_NONE, len);
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, len);
            ...
            for (int i = 0; i < len; i++) {
                int64_t x = xLocal(i);
                int64_t y = broadcastTmpTensor(i);
                if (x < y) {
                    zLocal(i) = x;
                } else {
                    zLocal(i) = y;
            ...
            // LocalTensor<int32_t> tmpTensor1 = tBuf[2].Get<int32_t>();
            // LocalTensor<int32_t> tmpTensor2 = tBuf[3].Get<int32_t>();
            // LocalTensor<int32_t> tmpTensor3 = tBuf[4].Get<int32_t>();
            ...
            LocalTensor<float> tmpTensor1 = tBuf[2].Get<float>();
            LocalTensor<float> tmpTensor2 = tBuf[3].Get<float>();
            LocalTensor<float> tmpTensor3 = tBuf[4].Get<float>();
...
template <typename T> class KernelFmin<T, 2> {
public:
    __aicore__ inline KernelFmin() {}
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, FminTilingData tiling)
    {
        ...
        if constexpr (std::is_same_v<T, bfloat16_t>) {
            ...
            pipe.InitBuffer(tBuf[2], this->tileLength * lastDimLength * sizeof(half));
            pipe.InitBuffer(tBuf[3], this->tileLength * lastDimLength * sizeof(half));
            pipe.InitBuffer(tBuf[4], this->tileLength * lastDimLength * sizeof(half));
        } else if constexpr (std::is_same_v<T, int64_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * lastDimLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[3], this->tileLength * lastDimLength * sizeof(int32_t));
            pipe.InitBuffer(tBuf[4], this->tileLength * lastDimLength * sizeof(int32_t));
        }
    }
    ...
    __aicore__ inline void Compute(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
                      std::is_same_v<T, int32_t>) {
            AscendC::Min(zLocal, xLocal, broadcastTmpTensor, compute_len);
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            LocalTensor<half> tmpTensor1 = tBuf[2].Get<half>();
            LocalTensor<half> tmpTensor2 = tBuf[3].Get<half>();
            LocalTensor<half> tmpTensor3 = tBuf[4].Get<half>();

            AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Cast(tmpTensor2, broadcastTmpTensor, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, compute_len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, compute_len);
            ...
            // AscendC::Cast(tmpTensor2, broadcastTmpTensor, AscendC::RoundMode::CAST_NONE, compute_len);
            // AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, compute_len);
            // AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_NONE, compute_len);
        } else if constexpr (std::is_same_v<T, bfloat16_t>) {
            LocalTensor<float> tmpTensor1 = tBuf[2].Get<float>();
            LocalTensor<float> tmpTensor2 = tBuf[3].Get<float>();
            LocalTensor<float> tmpTensor3 = tBuf[4].Get<float>();
    ...
    __aicore__ inline void ComputeB2(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
                      std::is_same_v<T, int32_t>) {
            AscendC::Min(zLocal, xBroadcast, yBroadcast, compute_len);
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            LocalTensor<half> tmpTensor1 = tBuf[2].Get<half>();
            LocalTensor<half> tmpTensor2 = tBuf[3].Get<half>();
            LocalTensor<half> tmpTensor3 = tBuf[4].Get<half>();

            AscendC::Cast(tmpTensor1, xBroadcast, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Cast(tmpTensor2, yBroadcast, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, compute_len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, compute_len);

        } else if constexpr (std::is_same_v<T, int64_t>) {
            for (int i = 0; i < compute_len; i++) {
                int64_t x = xBroadcast(i);
                int64_t y = yBroadcast(i);
                if (x < y) {
                    zLocal(i) = x;
                } else {
                    zLocal(i) = y;
...
template <typename T> class KernelFmin<T, 3> {
public:
    __aicore__ inline KernelFmin() {}
...
template <typename T> class KernelFmin<T, 4> {
public:
    __aicore__ inline KernelFmin() {}
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR z, FminTilingData tiling)
    {
        ...
        pipe.InitBuffer(inQueueX, BUFFER_NUM, tileLength * xShapeNew[dimnum - 1] * sizeof(T));
        pipe.InitBuffer(inQueueY, BUFFER_NUM, yShapeNew[dimnum - 1] * sizeof(T));
        pipe.InitBuffer(outQueueZ, BUFFER_NUM, tileLength * zShapeNew[dimnum - 1] * sizeof(T));

        // printf("tile*lastdim:%d\n",tileLength*lastDimLength);
        pipe.InitBuffer(tBuf[0], this->tileLength * lastDimStride * sizeof(T));
        pipe.InitBuffer(tBuf[1], this->tileLength * lastDimStride * sizeof(T));
        if constexpr (std::is_same_v<T, bfloat16_t>) {
            ...
            pipe.InitBuffer(tBuf[4], this->tileLength * lastDimStride * sizeof(float));
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * lastDimStride * sizeof(half));
            pipe.InitBuffer(tBuf[3], this->tileLength * lastDimStride * sizeof(half));
            pipe.InitBuffer(tBuf[4], this->tileLength * lastDimStride * sizeof(half));
        } else if constexpr (std::is_same_v<T, int64_t>) {
            pipe.InitBuffer(tBuf[2], this->tileLength * lastDimStride * sizeof(int32_t));
            pipe.InitBuffer(tBuf[3], this->tileLength * lastDimStride * sizeof(int32_t));
            pipe.InitBuffer(tBuf[4], this->tileLength * lastDimStride * sizeof(int32_t));
        }
    }
    ...
    __aicore__ inline void Compute(uint32_t len)
    {
        ...
        uint32_t dstShape[] = {len, lastDimStride};
        uint32_t srcShape[] = {1, lastDimStride};
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, uint8_t> ||
                      std::is_same_v<T, int8_t>) {
            BroadCast<T, 2, 0>((broadcastTmpTensor), (yLocal), dstShape, srcShape);
        } else {
            MyBroadCast<T, 2, 0>((broadcastTmpTensor), (yLocal), dstShape, srcShape);
        }

        uint32_t compute_len = len * lastDimStride;
    ...
    __aicore__ inline void Process2D(uint32_t basex, uint32_t basey, uint32_t basez)
    {
        ...
        int offset = (tileNum - 1) * tileLength * lastDimLength;
        // printf("tileLength:%d,LastDimLength:%d,LastTileLength:%d,offset:%d\n", tileLength, lastDimLength,
        //        lastTileLength, offset);
...
template <typename T> class KernelFmin<T, 5> {
public:
    __aicore__ inline KernelFmin() {}
    ...
    __aicore__ inline void ComputeB(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
                      std::is_same_v<T, int32_t>) {
            AscendC::Mins(zLocal, xLocal, yScale, len);
        } else if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
            LocalTensor<T> yBroadCast = tBuf[1].Get<T>();
            LocalTensor<half> tmpTensor1 = tBuf[2].Get<half>();
            LocalTensor<half> tmpTensor2 = tBuf[3].Get<half>();
            LocalTensor<half> tmpTensor3 = tBuf[4].Get<half>();

            // float yF32 = yScale*1.f;

            AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, len);
            // AscendC::Duplicate(yBroadCast, yScale, len);
            for (int i = 0; i < len; i++)
                yBroadCast(i) = yScale;
            AscendC::Cast(tmpTensor2, yBroadCast, AscendC::RoundMode::CAST_NONE, len);
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, len);
            // AscendC::Mins(tmpTensor3, tmpTensor1, (half)yF32, len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, len);
            ...
            float yF32 = ToFloat(yScale);
            AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Mins(tmpTensor3, tmpTensor1, yF32, compute_len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, compute_len);
        }
    ...
    __aicore__ inline void Compute(uint32_t len)
    {
        ...
        if constexpr (std::is_same_v<T, half> || std::is_same_v<T, float> || std::is_same_v<T, int16_t> ||
            ...
            AscendC::Cast(tmpTensor1, xLocal, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Cast(tmpTensor2, yLocal, AscendC::RoundMode::CAST_NONE, compute_len);
            AscendC::Min(tmpTensor3, tmpTensor1, tmpTensor2, compute_len);
            AscendC::Cast(zLocal, tmpTensor3, AscendC::RoundMode::CAST_RINT, compute_len);
        }
...
extern "C" __global__ __aicore__ void fmin(GM_ADDR x, GM_ADDR y, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling)
{
    GET_TILING_DATA(tiling_data, tiling);
    // TODO: user kernel impl
    if (TILING_KEY_IS(0)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 0> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 0> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }

    } else if (TILING_KEY_IS(1)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 1> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 1> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }

    } else if (TILING_KEY_IS(2)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 2> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 2> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }

    } else if (TILING_KEY_IS(3)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 3> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 3> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }

    } else if (TILING_KEY_IS(4)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 4> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 4> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }

    } else if (TILING_KEY_IS(5)) // no broadcast
    {
        if (std::is_same_v<DTYPE_X, bool>) {
            KernelFmin<uint8_t, 5> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        } else {
            KernelFmin<DTYPE_X, 5> op;
            op.Init(x, y, z, tiling_data);
            op.Process();
        }
    }

...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    if (need_broadcast) {
        ...
        if (dimnum == 1) // broadcast -1(last) dim, block cut at first dim, tile cut at -1 dim
        {
            ...
            block_num = ascendcPlatform.GetCoreNumAiv();
            ...
            int max_tile_len = 4096 / (1 + zShapeNew[dimnum - 1]);
            if (max_tile_len < 1)
                max_tile_len = 1;
        ...
        {
            int max_tile_len = 8192 / (2 * ROUND_UP(zShapeNew[dimnum - 1] * dataWidth, 32) / dataWidth);
            if (max_tile_len < 1)
                max_tile_len = 1;
            ...
            tilingKey = 4;
            // if((zShapeNew[dimnum - 1] * dataWidth)%32==0) tilingKey = 20;
        }

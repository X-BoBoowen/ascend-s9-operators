...
static ge::graphStatus TilingFunc(gert::TilingContext *context)
{
    ...
    TilingData tiling;
    uint32_t dt = (context->GetInputDesc(0)->GetDataType() == ge::DT_FLOAT16);
 ...
 {
     // 广播shape：output shape为两个输入的广播结果
     ...
     for (size_t i=0; i<ndim_out; ++i) {
         ...
         if (d1 != d2 && d1 != 1 && d2 != 1) return GRAPH_FAILED; // 不能广播

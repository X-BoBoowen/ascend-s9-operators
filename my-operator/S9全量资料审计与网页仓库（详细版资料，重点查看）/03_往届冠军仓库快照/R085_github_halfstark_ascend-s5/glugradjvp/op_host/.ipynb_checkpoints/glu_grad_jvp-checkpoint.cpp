...
//     data_sz *= x1_shape->GetStorageShape().GetDim(i);
    ...
    uint32_t dims = context->GetInputShape(2)->GetStorageShape().GetDimNum();
    uint32_t totalLength = context->GetInputTensor(2)->GetShapeSize();
    uint32_t typeSize = GetSizeByDataType(context->GetInputDesc(1)->GetDataType()); //输入类型
    uint32_t totalBytes = context->GetInputTensor(2)->GetShapeSize();
    ...
    // printf("tileDataNum %d ubDataNumber:%d\n", tileDataNum,ubDataNumber);
    ...
    for(int i=0;i<dims;i++) {
        shapeDim[i] = context->GetInputShape(2)->GetStorageShape().GetDim(i);
        // printf("shape i:%d %d\n", i, shapeDim[i]);
    ...
    // printf("iterstep:%d everyCoreInputBlockNum:%d tileBlockNum:%d padtimes:%d\n", iterStep,everyCoreInputBlockNum,tileBlockNum,padtimes);

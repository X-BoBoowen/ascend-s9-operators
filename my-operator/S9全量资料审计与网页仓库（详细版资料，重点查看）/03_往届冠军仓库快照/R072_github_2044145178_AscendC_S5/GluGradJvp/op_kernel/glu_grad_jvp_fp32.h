...
constexpr uint16_t BufferNum = 2;
class KernelGluGradJvpFp32 {
public:
    __aicore__ inline KernelGluGradJvpFp32() {}
    __aicore__ inline void Init(GM_ADDR x_grad, GM_ADDR y_grad, GM_ADDR x, GM_ADDR v_y, GM_ADDR v_x, GM_ADDR jvp_out, int HI, int J, int KS, uint32_t smallSize, uint32_t incSize, uint16_t formerNum,
        ...
        uint32_t spaceSize = min((uint32_t)(hi_block_size * sizeof(float)), (uint32_t)(160 / 8) * 1024 / BufferNum);
        ...
        pipe->InitBuffer(x1Buf, BufferNum, spaceSize);
    ...
    __aicore__ inline void Process_iter(uint32_t inputIndex, uint32_t glu_outIndex, uint32_t iterSize) {
        ...
        DataCopyPad(v_yLocal, v_yGm[glu_outIndex], copyParams, padParams);
    ...
    __aicore__ inline void Process() {
        ...
        for (uint32_t i = 0; i < bigLoopTimes; i++) {
            ...
            for (uint32_t j = 0; j < smallLoopTimes; j++) {
                ...
                //     printf("KernelGluJvpFp32 Process: blockBeginIndex=%d, iterInputIndex=%d, iterSize=%d\n", blockBeginIndex, iterInputIndex, iterSize);
                // }

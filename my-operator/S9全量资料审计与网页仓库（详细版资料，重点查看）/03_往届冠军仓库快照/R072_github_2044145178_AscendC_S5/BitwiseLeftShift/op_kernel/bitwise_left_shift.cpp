...
#include "bitwise_left_shift_32_big.h"
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR input, GM_ADDR other, GM_ADDR out, uint32_t smallSize, uint32_t incSize, uint16_t formerNum, TPipe *pipeIn) {
        ...
        uint32_t spaceSize = min((uint32_t)(size * sizeof(T)), (uint32_t)(63) * 1024 / BufferNum_k);
    ...
    __aicore__ inline void Process() {
        ...
        int power = 1;
        for (int i = 0; i < n; i += 4) {
            powerLocal.SetValue(i, power);
            power <<= 1;
            powerLocal.SetValue(i + 1, power);
            power <<= 1;
            powerLocal.SetValue(i + 2, power);
            power <<= 1;
            powerLocal.SetValue(i + 3, power);
            power <<= 1;
        }
...
extern "C" __global__ __aicore__ void bitwise_left_shift(GM_ADDR input, GM_ADDR other, GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling) {
    ...
    if (TILING_KEY_IS(1)) {
        ...
        KernelBitwiseLeftShift_32_big<int32_t> kernel;
        kernel.Init(input, other, out, tiling_data.smallSize, tiling_data.incSize, tiling_data.formerNum, tiling_data.totalSize, &pipe);
        kernel.Process();
    }
}
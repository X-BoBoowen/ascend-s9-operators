...
$completeCsv = Join-Path $packageDir '00_材料索引\外部链接总表_完整.csv'
$initialResultCsv = Join-Path $packageDir '00_材料索引\网页获取结果.csv'
$archiveRoot = Join-Path $packageDir '02_网页归档\补充网页'
$statusRoot = Join-Path $packageDir '00_材料索引\补充网页获取状态'
New-Item -ItemType Directory -Path $archiveRoot -Force | Out-Null
New-Item -ItemType Directory -Path $statusRoot -Force | Out-Null

$already = @{}
foreach ($row in (Import-Csv -LiteralPath $initialResultCsv)) { $already[$row.OriginalUrl] = $true }
$rows = Import-Csv -LiteralPath $completeCsv | Where-Object { -not $already.ContainsKey($_.OriginalUrl) }
...
$jobs = for ($i = 0; $i -lt $rows.Count; $i++) {
    ...
    [pscustomobject]@{
        ...
        Url = $row.OriginalUrl
        FileName = ('S{0:D3}.html' -f ($i + 1))
    }
}
$jobs | Export-Csv -LiteralPath (Join-Path $packageDir '00_材料索引\补充网页归档目录映射.csv') -NoTypeInformation -Encoding utf8BOM
...
$jobs | ForEach-Object -Parallel {
    ...
    $statusPath = Join-Path $statusRoot ('S{0:D3}.json' -f [int]$job.Index)
    $started = Get-Date
    $status = 'Failed'
    $httpStatus = ''
    $finalUrl = ''
...
$results = Get-ChildItem -LiteralPath $statusRoot -Filter 'S*.json' -File |
    Sort-Object Name |
    ForEach-Object { Get-Content -LiteralPath $_.FullName -Raw | ConvertFrom-Json }
$results | Export-Csv -LiteralPath (Join-Path $packageDir '00_材料索引\网页获取结果_补充.csv') -NoTypeInformation -Encoding utf8BOM

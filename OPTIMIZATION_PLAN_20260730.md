...
| SquareSumV1 | SquareSumV1-S01 | 7.72 | 404.592 | 241.66 | 1720.188 | 908.376 | 3282.536 | 71.07% |
...
| SquareSumV1 | 10 名以后 | 0 | 3282.536 | 1268.598 | 待填 | 待填 | 第10名 | +10 |

### 3.2 第 10 名差距

| 题目 | 当前值 | 第10名 | 至少需下降 | 至少需下降比例 | 当前/门槛 |
|---|---:|---:|---:|---:|---:|
| SquareSumV1 | 3282.536 | 1268.598 | 2013.938 | 61.35% | 2.59× |
...
|---|---|---:|---:|---:|
| SquareSumV1 | Case4 + Case5 | 2628.564 | 614.626 | 4.28× |
...
| SquareSumV1 Case4 | 当前具体 fastPath 未确认；可能为 strided/grouped 路径 | E | S02A 后先做通用路径 microbenchmark | 优化错误路径 | 不依据隐藏 shape；以本地覆盖矩阵选路径 |
...
当前状态：等待官方五个隐藏 Case 评测。平台结果返回前，不更新
SquareSumV1 的最低 `prof_sum`，不声称已经进入前十。

若平台 Case4 明显下降，则继续沿 fastPath4/相邻通用路径完善；若 Case4
基本不动，则优先实施 S02F（fastPath3 输出维向量化）。Case5 仍高则继续
S02G（`ReduceContiguous` 分层归约 microbenchmark），不得猜测隐藏 shape。

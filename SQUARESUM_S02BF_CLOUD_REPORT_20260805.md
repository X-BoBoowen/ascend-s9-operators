...
请只上传上述 S02BF ZIP，并返回 Case1～Case5 原始结果与 `prof_sum`。
若 5/5 Pass，则逐 Case 与 S02BA 比较；只有稳定中位总耗时不高于
`1195.599 us` 才满足本轮 0.30 目标。若未达标，继续从未被该布局路由
覆盖的通用慢路径做 profiling，不反推或硬编码隐藏数据。

...
> 为 fastPath4 通用低并行度区域增加 40 核、二维批量搬运的 Split-K。
> S02CR 路由契约 `12/12`、坐标与数值模型 `45/45` 通过，但两者均尚未在
...
| SquareSumV1 | S02CQ/S02CR 独立本地候选，待云端门禁 | CQ 模型 27/27；CR 路由 12/12 + 模型 45/45；真机未运行 | 禁止打包 | S02CN 4010.864 已淘汰；正式最好 S02CA 3982.569 |
...
并且部分 shape 超出题面维度上限。S02CQ 与 S02CR 是复核后建立的两个独立
本地候选；前者处理 BF16 中间量化，后者处理 fastPath4 的通用低并行度。
两者均尚无 910B 构建、正确性或性能结果，禁止打包提交。
...
执行 `diagnostics/run_squaresum_s02cr_cloud_gate_20260806.sh`，因为它针对
毫秒级结构瓶颈；随后独立执行 S02CQ。两套门禁都要求同机隔离 A→B→A、
单点回退不超过 3%、基线漂移不超过 3%；S02CR 的目标矩阵总改善门槛为
10%，S02CQ 为 5%。

...
> S02CA 的 `3982.569 μs` 回退 `0.7105%`，已淘汰。复核发现旧性能闭环
> 混用了非官方 Event 口径、部分题面域外 shape，并遗漏 BF16 性能门禁。
> 当前本地候选 S02CQ 仅删除 BF16 平方结果的中间量化往返；已通过静态
> 审计和 `27/27` 数值模型，但尚未在 910B/CANN 8.5.0 构建或运行，禁止
> 提交。正式基线为 S02CA `3982.569 μs`，总目标仍为
> `≤1195.599 μs`。证据见
> [`SQUARESUM_S02CQ_LOCAL_AUDIT_20260806.md`](./SQUARESUM_S02CQ_LOCAL_AUDIT_20260806.md)。
...
| SquareSumV1 | S02CQ 本地候选，待云端门禁 | 静态通过 + BF16 模型 27/27；真机未运行 | 禁止打包 | S02CN 4010.864 已淘汰；正式最好 S02CA 3982.569 |
...
并且部分 shape 超出题面维度上限。S02CQ 是复核后建立的 BF16 融合本地
候选，只通过静态检查和 `27/27` 数值模型，尚无 910B 构建、正确性或性能
结果，禁止打包提交。

当前不能确认的是：隐藏 Case 的具体 shape/dtype/TilingKey。后续优化必须
继续依据通用布局、公开矩阵和真机 profiling，不得把平台 Case 当作可猜测
或可硬编码的数据；
只有官方五项结果可以证明是否达到 `0.30 × T_baseline`。下一次云端首先
执行 `diagnostics/run_squaresum_s02cq_cloud_gate_20260806.sh`；门禁要求
同机隔离 S02CA-A → S02CQ → S02CA-B、总改善至少 5%、单点回退不超过
3%、基线漂移不超过 3%。

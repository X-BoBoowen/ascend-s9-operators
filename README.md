...
>
> 当前阶段：SquareSumV1-S02BK 已完成 910B4/CANN 8.5.0 构建、
> `1370/1370` 既有全量门禁和 `45/45` 小宽度专项门禁，等待官方评测。
> 它针对 fastPath2 的 `inner=1..7` 小宽度归约消除逐行 32B 填充，专项
> 45 项合计由 `4676.563 μs` 降至 `2661.409 μs`（`1.757x`）；真正受益
> 的 `inner=2..7` 单项最高 `6.274x`。正式优化基线仍为 S02BA 的
> `3985.330 μs`，目标为 `≤1195.599 μs`。证据见
> [`SQUARESUM_S02BK_CLOUD_REPORT_20260806.md`](./SQUARESUM_S02BK_CLOUD_REPORT_20260806.md)。
...
| SquareSumV1 | S02BK 待官方评测 | S02BK 1370/1370 + 45/45 小宽度专项 | S02BK 已独立打包审计 | S02BK 待测；S02BI 3982.789 未晋级，正式基线仍为 S02BA 3985.330 |

...
S02BS 在 fastPath4、`inner=1`、末级归约长度为 2～64 的 2 次幂且至少
形成 32 个分组任务时，将 8 个物理连续输出行合并搬运，并用
`WholeReduceSum` 同时得到 8 个 partial。其他路径保持 S02BP/S02BN
实现；key 5 仅在 `fastPath=4` 时进入新内核，原 fastPath3 行为不变。

热点三种 dtype 合计从 `6615.292` 降到 `140.604 μs`（`47.049x`）；
12 个规模交叉点全部加速，合计从 `5681.952` 降到 `371.264 μs`
（`15.304x`）。累计 `1613/1613` 正确。

```text
SQUARESUM_S02BS_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02BS\SquareSumV1.zip
大小：687510 bytes
ZIP SHA-256：
65CA55C54F3FE3A388411A109C024A7101EA0BC66E40BE4959179AF5396409FE
RUN SHA-256：
5DABA1D2AF285683D8BD755A549EB112145126EDFE04A815A18A461B0C5FD409
```

S02BS 官方结果为 `7.416 / 407.388 / 139.676 / 2538.380 /
903.168 us`，`prof_sum=3996.028 us`。相对 S02BA 慢 `10.698 us`，
不晋级。

### 12.16 SquareSumV1-S02BT fastPath4 窄 inner 自适应分组候选

S02BT 将 S02BS 的 fastPath4 行合并从 `inner=1` 扩展到
`inner=2～16`。Host 根据 UB 容量和并行任务数选择 `8/4/2/1` 行分组；
Kernel 为每个 inner 行补齐到 32 字节，在对齐 FP32 临时区完成树归约，
再用标量安全地紧密打包输出。路由只依赖布局、dtype、容量和任务数。

33 点矩阵合计从 S02BS 的 `11680.390` 降到 `7045.946 μs`
（`1.658x`）；`inner=2/4/8/16` 分别加速
`1.507x / 1.713x / 2.118x / 1.316x`。最终累计 `1634/1634` 正确。

```text
SQUARESUM_S02BT_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02BT\SquareSumV1.zip
大小：713017 bytes
ZIP SHA-256：
5E1AAEC27C8465B44695130C6985D873C49ACD807A6AD76CE8FF31C19D3DCCF4
RUN SHA-256：
87D7C68EC6F8C39D133C6BA5B4256C3F0D626A83755A27755EFC170344519D72
```

S02BT 官方结果为 `7.448 / 395.728 / 139.240 / 2548.652 /
904.312 us`，`prof_sum=3995.380 us`。相对 S02BA 慢 `10.050 us`，
不晋级。
...
S02CA 官方结果为 `6.410 / 390.238 / 138.633 / 2557.111 /
890.177 us`，`prof_sum=3982.569 us`。相对 S02BA 只快 `2.761 us`
（`0.069%`），属于平台波动量级，不作为有效突破。

### 12.18 SquareSumV1-S02CD 紧凑分组与 split-K 候选

S02CD 针对性能图谱中 fastPath4 的通用低吞吐结构重写数据通路：
`inner=2` 的短归约采用按输出并行的连续整块搬运，长归约采用 40 核
split-K；`inner=4/8/16` 的大规模 split-K 也改为连续整块搬运。
`lastReduce=2, inner=2` 因 FP32 中间行仅 16B，严格回退到原 32B 对齐
路径。所有路由只使用 shape、axis、stride、dtype 和 UB 容量。

最终合并版通过既有自适应边界 `69/69`、S02CA split-K 边界
`36/36`、新增 inner2 边界 `42/42`，合计 `147/147`；随后 42 点
全路径图谱再次 `42/42`。相同图谱相对 S02CA：

```text
42 点中位耗时总和：16965.913 -> 3459.513 us（4.904x）
单点最大中位耗时： 3291.287 ->  152.547 us（21.575x）
```

```text
SQUARESUM_S02CD_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CD\SquareSumV1.zip
大小：829902 bytes
ZIP SHA-256：
5D847711252AE49F5CA6BA8B65C14983BE703D4AE0493D323B5B61A90EF79D30
RUN SHA-256：
39D38CBA228B62E3261A823CFE9E9394E632B21A83E67CEA63531F48299E2489
```

S02CD 官方结果为 `6.390 / 397.4975 / 139.603 / 2556.431 /
894.4275 us`，`prof_sum=3994.349 us`。相对 S02BA 慢 `9.019 us`
（`0.226%`），说明上述公开 fastPath4 图谱改善没有命中正式负载。

### 12.19 SquareSumV1-S02CE fastPath3 输出 9～32 split-K 候选

S02CE 修复 fastPath3 中“输出数可整除 8 时仅按输出组启动少量核”的
并行度瓶颈。对于输入规模、归约规模、连续尾段和归约行数均达到通用
门限，且输出数为 9～32 的布局，改用已有的 40 核 workspace split-K
路径；门限只依赖 shape、axis、stride 和输入规模，不包含官方 Case
信息。

公开代表布局 `(64, 4, 8, 2048), axes=(0, 3)` 的独立 A/B 中，FP16/FP32
中位耗时由 S02CD 的 `150.160/152.547 us` 降至 `55.716/54.340 us`。
完整 42 点图谱复验中该点为 `66.120/64.727 us`，图谱总体变化为：

```text
42 点中位耗时总和：3459.513 -> 2985.773 us（下降 13.70%）
单点最大中位耗时： 152.547 ->  124.940 us
退化超过 15% 的点：0
```

S02CE 通过新增输出 9～32 及门限外回退用例 `42/42`、原有三组回归
`147/147`、全路径图谱 `42/42`。

```text
SQUARESUM_S02CE_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CE\SquareSumV1.zip
大小：829605 bytes
ZIP SHA-256：
BBE62860C2DB8891F5A6699FEFF649C46CEBF9D935BD23D2C81633B4C3EB1D70
RUN SHA-256：
FE7DAAB7DA5D4162ABA12F613B412D6BF8D263827EB95316D769889BA9C66B35
```

S02CE 尚无官方五 Case 结果；公开图谱不能替代正式平台证据。

### 12.20 SquareSumV1-S02CF 大规模窄 inner 自适应 split-K 候选

S02CF 根据 32M 元素全 fastPath 图谱修复 fastPath4 的大规模结构性退化。
此前 inner=4/8/16 的 split-K 只有“宽度 8 的整组归约行能装入 8192 元素
UB”时才启用；归约尾较长时会回退到低并行度的逐行填充路径。S02CF
按 UB 容量把分组宽度自适应降为 8/4/2/1，并为单行最多 16384 元素增加
长块 tiling key。路由仍只依赖公开 shape/axis/stride/dtype/容量条件。

相同 26 点、约 32M 元素大规模图谱的结果：

```text
中位耗时总和：27307.280 -> 7228.150 us（下降 73.53%）
最大单点：    11950.520 ->  813.140 us

inner8 FP16：11950.520 -> 148.790 us（80.318x）
inner8 FP32： 2371.090 -> 240.720 us（ 9.850x）
rank5  FP16： 4178.150 -> 134.870 us（30.979x）
rank5  FP32： 2424.400 -> 221.580 us（10.941x）
```

S02CF 通过新增大规模门限 `42/42`、累计五组正确性回归 `231/231`、
大规模图谱 `26/26` 和小规模全路径图谱 `42/42`。小图谱中位总和的
百微秒级差异存在明显运行噪声；大规模目标点的 10～80 倍收益多轮复验
稳定。

```text
SQUARESUM_S02CF_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CF\SquareSumV1.zip
大小：865636 bytes
ZIP SHA-256：
26693F15A8D7434D840018E948F66CD21E13BCBCF59F9A46617251407E09EFC3
RUN SHA-256：
CAD65A62CA89AE93FB6A237F5161EC322A2A407ECEFE0F47F2F695774DDC729F
```

S02CF 尚无官方五 Case 结果；它取代 S02CE 作为当前优先测评候选。

### 12.21 SquareSumV1-S02CG fastPath3 输出 33～64 split-K 候选

S02CG 在 S02CF 基础上只扩大一个已经验证的通用路由门限：fastPath3
非连续归约的 workspace split-K 输出上限由 32 提高到 64。输入规模、
连续尾段、归约行数等安全条件及 kernel 均保持不变；输出 65 和归约行数
15 会继续回退旧路径。

相同 26 点、约 32M 元素大规模图谱的结果：

```text
中位耗时总和：7228.150 -> 6068.880 us（下降 16.04%）
目标 FP16：    549.300 ->  111.910 us（4.908x）
目标 FP32：    621.350 ->  177.080 us（3.509x）
```

S02CG 通过新增边界 `39/39`、累计六组正确性回归 `270/270`、大规模
图谱 `26/26` 和小规模全路径图谱 `42/42`。S02CH 的 inner=65 长块实验
只有约 1%～2% 改善，已明确淘汰，没有混入本候选。

```text
SQUARESUM_S02CG_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CG\SquareSumV1.zip
大小：865603 bytes
ZIP SHA-256：
C31BABBC0802FD89B8BB9ED3B85294876338C84699AFA61D8813DD39EB6E1F48
RUN SHA-256：
6936CE6EF7BB93BA81277C08069D12CED2E0A79C9C2A77A77D26505632B796F6
```

S02CG 尚无官方五 Case 结果；公开探针只证明改动本身有效，是否命中正式
负载仍以平台结果为准。

### 12.22 SquareSumV1-S02CL fastPath4 完整行分核候选

S02CF 官方 `3994.349 us` 与 S02CD 完全相同，证明其公开图谱收益没有命中
官方主要路径。S02CL 改为优化另一条通用结构：fastPath4、`inner > 16`、
大输入且输出由 16～40 个完整 inner 行组成时，按完整输出行分核。FP32
非对齐 inner 仍保持 40 核，门限外布局全部回退旧逻辑。

代表性独立 A/B：

```text
31 行 inner=65  FP16：422.56 -> 333.40 us
24 行 inner=256 FP32：135.47 ->  90.69 us
40 行 inner=33  FP16：883.98 -> 560.25 us
rank5 inner=128 FP32：158.94 ->  78.73 us
```

S02CL 通过新增边界 `45/45`、累计七组正确性 `315/315`、大规模图谱
`26/26` 和小规模图谱 `42/42`。正式平台结果仍是唯一晋级依据。

```text
SQUARESUM_S02CL_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CL\SquareSumV1.zip
大小：865936 bytes
ZIP SHA-256：
A4C82815EF2FAF8E96FC6D310EAC0EE4CC17D41CD0ECE3FF1DEE38F23891044B
RUN SHA-256：
459E63207CADD4226537F0168D318855BC41B55AA8E14C23EF8CAC9BA7643F06
```

S02CL 官方结果为 `6.470 / 388.257 / 139.113 / 2558.891 /
893.908 us`，`prof_sum=3986.639 us`。相对 S02BA 慢 `1.309 us`
（`+0.033%`），正式淘汰。

### 12.23 SquareSumV1-S02CM 非对齐完整行连续归约候选

S02CM 重写 fastPath4 非 32 字节对齐完整行的数据通路。旧实现对每一行
分别搬运、补齐并按 padded stride 归约；新实现将完整的 `rows × inner`
合并为一次连续搬运，只在整块末尾 padding，并把任意实际行数分解为
2 次幂块、归约到 8 行后统一完成最终合并。新路径只在 `inner <= 256`
时启用，257 以上及非完整行严格回退；路由仅使用 shape、axis、dtype、
对齐与 UB 容量。

首次累计回归抓到 BF16 使用补齐行数而非实际行数的错误，修正后从头通过
累计正确性 `315/315`、非对齐扫宽 `30/30`、大规模图谱 `26/26` 和
小规模图谱 `42/42`。相同 26 点、约 32M 元素图谱结果：

```text
中位耗时总和：5991.740 -> 4996.010 us（下降 16.62%）
inner65 FP16： 634.220 ->  144.570 us（4.387x）
inner65 FP32： 746.140 ->  225.670 us（3.306x）
```

```text
SQUARESUM_S02CM_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CM\SquareSumV1.zip
大小：871273 bytes
ZIP SHA-256：
15CB455648368C37098C871790488497DAE433D1D4FE941B16615BB65117D1E0
RUN SHA-256：
E58C9FC7EA08C73E81F50C902C7074AEFD9E74D9641E82F5764776F6E7E151B3
```

S02CM 官方结果为 `6.550 / 400.108 / 139.1025 / 2546.821 /
895.208 us`，`prof_sum=3987.7895 us`。相对 S02BA 慢 `2.4595 us`
（`+0.0617%`），正式淘汰。

### 12.24 SquareSumV1-S02CN fastPath2 完整输出行候选

S02CN 修复连续中间轴归约的通用结构性瓶颈。旧 fastPath2 按输出元素
分核，核边界可能落在 inner 行中间，并对每个归约行执行短 DMA、逐行
padding 和 padded-stride 归约。新 TilingKey 15 改为按完整输出行分核，
将每个 tile 的 `reduce rows × inner` 合并为一次连续 DMA，只在整块尾部
补齐，并复用任意行数到 8 行的树归约。

路由仅依据 fastPath2、输入规模、归约长度、inner、完整输出行数和 UB
容量；当前通用范围为输入不少于 `2^20`、归约不少于 512、inner 9～129、
输出行不少于 40。边界、rank-4/5、keep_dims、负轴/无序轴和 FP16/BF16/
FP32 专项通过 `96/96`，七套历史累计回归通过 `315/315`。16M 公共矩阵
的目标宽度取得约 2～21× 加速；32M、inner=64 的 FP16/FP32 分别从
`299.660/404.460 us` 降至 `107.670/211.650 us`。

```text
SQUARESUM_S02CN_CLOUD_REPORT_20260806.md

D:\29722\Desktop\GCC\提交相关材料\20260806\S02CN\SquareSumV1.zip
大小：877998 bytes
ZIP SHA-256：
1D04777575176A168860CDA1BE96B2F75779C7FAAEF401E78DFA4F8845D28E16
RUN SHA-256：
4E72EA2372B9C94719610D1343C8E1D4B6F1B220AC7A68B5C61497EA85A778B3
```

S02CN 官方结果为 `7.448 / 409.292 / 140.352 / 2550.192 /
903.580 us`，`prof_sum=4010.864 us`。相对 S02CA `3982.569 us` 回退
`28.295 us / 0.7105%`，正式淘汰。旧公共图谱还混入了题面维度范围外的
shape，因此不得再用作提交依据。

### 12.25 SquareSumV1-S02CQ/S02CR/S02CS/S02CT 本地候选

重新按题面 Excel、官方公开 case、计时扩展和 CANN 8.5.0 API 逐项复核后，
当前保留四个互不叠加的候选：

- S02CQ：仅测试 BF16 平方后的中间量化往返是否为主要成本；
- S02CR：为 fastPath4、通用大归约且低并行度的布局增加二维批量搬运
  Split-K；
- S02CS：把 S02CA 已经真机验证过的 fastPath3 `reduceMode=3` 补到此前
  被 `tail>=1024` 门限遗漏的短尾 `1..1023`，并以精确分核成本门槛覆盖
  输出 `1..16`；
- S02CT：把 fastPath1 已有的 40 核 `reduceMode=2` 从输出 `1..8` 扩展到
  `1..16`，源码差异只有一行 Host 常量。

S02CS 与 S02CT 的 Kernel 都与正式 S02CA 字节一致。本地公开维度矩阵
`32/32`；S02CS 路由/边界 `14/14`、三 dtype 数值模型 `21/21`；S02CT
路由/边界 `10/10`、三 dtype 数值模型 `15/15`。它们仍未在
...
源码哈希和 `.run` 0755 权限审计。平台正式成绩为 `3259.544 μs`，比
S02F 慢 `35.549 μs`，因此 S03J 已否决。
...
### 12.27 SquareSumV1-S03K fastPath2 完整输出行候选

S03J 正式成绩仍未命中主要隐藏耗时后，对 S03J 做全路径真机图谱复核。
`fastPath2` 的大规模小-inner 归约仅约 2.6～12.6 GB/s，根因是原
workspace split-K 让多个核反复遍历输出行，并产生大量短 2-D DMA。

S03K 仅对 `fastPath2`、输入不少于 1 Mi 元素、`reduce>=2048`、
`inner<=8`、完整输出行数不少于 40 的通用区域启用 key 6：每个核负责
完整输出行，以单块连续 DMA 分片搬运并在 UB 内完成平方归约。其余路径
保持 S03J 行为。

910B A/B/A 的 15 个目标点全部正确，聚合提升 `84.7826%`，单点最低
提升 `50.8800%`，两轮基线最大漂移 `2.2591%`；15 个阈值外控制点最大
表观回退 `2.0023%`。完整、随机、旧边界和新路径边界共
`1130/1130` 通过。详细证据见：

```text
SQUARESUM_S03K_CLOUD_GATE_20260807.md
...
S03K 正式平台结果为 `7.692 / 402.132 / 242.372 / 1721.900 /
903.044 μs`，`prof_sum=3277.140 μs`。五个 Case 均比 S03J 慢，合计
回退 `17.596 μs（0.5398%）`；相对历史最好 S02F 回退 `53.145 μs
（1.6484%）`。因此 S03K 已淘汰，局部 A/B/A 大幅改善再次证明不能替代
正式五 Case。

### 12.28 SquareSumV1-S03L fastPath4 相邻保留行合并搬运

重新检查完整图谱后，rank-5 非连续多轴归约的旧 fastPath4 仍会按每个
输出行、每个外层归约组发起短 DMA。S03L 新增 key 7，在输入不少于
1 Mi、归约不少于 2048、inner 为 8 的倍数、最内归约维为 2 次幂、UB
容量与任务数均满足通用门槛时，将物理相邻的 2/4/8 个保留输出行合成
一次连续搬运；奇数/非对齐/低并行及其他 fastPath 保持原实现。

S03K/S03L/S03K 的 15 点目标 A/B/A 从 `18763.6500 μs` 降到
`1950.2200 μs`，聚合改善 `89.6064%`，最差单点改善 `79.2200%`；
18 个控制点最大表观回退 `7.2830%`，两次控制基线最大漂移 `5.4654%`。
累计合法正确性 `1235/1235`。详细证据见：

```text
SQUARESUM_S03L_CLOUD_GATE_20260807.md
...
### 12.29 SquareSumV1-S03M/S03N/S03O 分阶段 fastPath4 候选

S03M 从干净 S02F 重建 rank-5 非连续多轴归约：key 7 将物理相邻的
2/4/8 个保留行合并为连续 DMA；S03N 把最内归约维扩展到任意合法长度；
S03O key 8 处理非 8 对齐的小 inner。初版 S03O 在 `inner=31/63` 和
奇数归约 127 上明显回退，因此最终实现只在两字节 dtype、
`inner <= 16`、最内归约为 2 次幂且全部容量/任务门槛满足时启用，其他
区域精确回退 S03N。

最终独立安装包通过 `1325/1325` 完整正确性。FP16/BF16 目标 A/B/A 从
`5186.4430 us` 降到 `2542.7710 us`，聚合改善 `50.9727%`，最差单点
改善 `50.5274%`，最大基线漂移 `0.2887%`。FP32、奇数归约以及
`inner=17/31/63` 五个关键回退点最大表观回退仅 `0.3094%`。

```text
SQUARESUM_S03O_CLOUD_GATE_20260807.md
...
以上仍是内部正式兼容 profiling，不是比赛平台五 Case。S03O 是当前唯一
推荐提交的新候选，但只有正式结果能够判断是否替代 S02F。
...
`3260.665 μs`、S03J 为 `3259.544 μs`、S03K 为 `3277.140 μs`，均已
淘汰。此前 Event 图谱不等价于官方评分口径，局部巨大收益也不能替代
正式五 Case。S03O 是当前唯一推荐提交的新候选：它通过 1325/1325
合法正确性，启用目标域 A/B/A 改善 50.9727%，关键回退控制最大表观
回退 0.3094%；源码、已验证 `.run` 与 ZIP 已固定，但尚无正式平台成绩。

当前不能确认的是：隐藏 Case 的具体 shape/dtype/TilingKey。后续优化必须
继续依据通用布局、公开矩阵和真机 profiling，不得把平台 Case 当作可猜测
或可硬编码的数据；
只有官方五项结果可以证明是否达到 `0.30 × T_baseline`。下一步提交
S03O 并逐 Case 对照 S02F/S03K；若目标路径没有映射到正式耗时，应回滚
S02F，继续做通用路径归因，不得靠扩大阈值或叠加未证实改动碰运气。

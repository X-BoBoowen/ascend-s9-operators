...
> S02CA 的 `3982.569 μs` 回退 `0.7105%`，已淘汰。评分口径复核后建立四
> 个相互独立的本地候选：S02CQ 删除 BF16 平方结果的中间量化往返；S02CR
> 为 fastPath4 通用低并行度区域增加 40 核、二维批量搬运的 Split-K；S02CS
> 将已有 fastPath3 Split-K 补到此前遗漏的短连续尾段 `1..1023`；S02CT
> 将 fastPath1 的同一已验证 Split-K 从输出 8 扩展到输出 16。
> S02CR 路由契约 `12/12`、坐标与数值模型 `45/45` 通过；S02CS 路由契约
> `14/14`、数值模型 `21/21` 通过；S02CT 路由 `10/10`、数值模型
> `15/15` 通过，但这些候选均尚未在
...
> 、[`SQUARESUM_S02CS_LOCAL_AUDIT_20260806.md`](./SQUARESUM_S02CS_LOCAL_AUDIT_20260806.md)
> 与 [`SQUARESUM_S02CT_LOCAL_AUDIT_20260806.md`](./SQUARESUM_S02CT_LOCAL_AUDIT_20260806.md)。
...
| SquareSumV1 | S02CQ/S02CR/S02CS/S02CT 独立本地候选，待云端门禁 | CQ 模型 27/27；CR 路由 12/12 + 模型 45/45；CS 路由 14/14 + 模型 21/21；CT 路由 10/10 + 模型 15/15；真机未运行 | 禁止打包 | S02CN 4010.864 已淘汰；正式最好 S02CA 3982.569 |

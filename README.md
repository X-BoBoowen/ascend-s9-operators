...
>
> 当前阶段：SquareSumV1-S02BT 已完成 910B4/CANN 8.5.0 构建、安装、
> A/B、累计 `1634/1634` 门禁和提交包审计，等待官方评测。它在 S02BS
> 的 `inner=1` 行合并基础上，新增 fastPath4 `inner=2～16` 的自适应
> `8/4/2/1` 行分组与对齐归约；33 点矩阵合计由 `11680.390 μs` 降至
> `7045.946 μs`（`1.658x`）。
> S02BK 官方 `3984.7995 μs` 只比基线快 `0.5305 μs（0.013%）`，判定
> 未晋级。正式优化基线仍为 S02BA 的
> `3985.330 μs`，目标为 `≤1195.599 μs`。证据见
> [`SQUARESUM_S02BT_CLOUD_REPORT_20260806.md`](./SQUARESUM_S02BT_CLOUD_REPORT_20260806.md)。
...
| SquareSumV1 | S02BT 待官方评测 | S02BT 累计 1634/1634 | S02BT 已独立打包审计 | S02BT 待测；S02BK 3984.7995 未晋级，正式基线仍为 S02BA 3985.330 |
...
当前能够确认的是：截至 S02CD 的官方候选均为 `5/5 Pass`，但都没有
形成相对 S02BA `3985.330 μs` 的有效稳定提升；新候选 S02CG 已通过
最终合并回归和全路径复验，尚无官方结果。

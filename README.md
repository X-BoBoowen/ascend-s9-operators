...
>
> 当前阶段：SquareSumV1-S02BF 已交付待测；其严格后继 S02BG 又完成
> 1295/1295 门禁、相邻安装 A/B 和独立包审计。S02BG 将 FP32 中间连续
> 归约宽度 8 提速 1.005～11.96 倍。正式对比基线仍为 S02BA 的
> `3985.33 μs`，目标为 `≤1195.599 μs`。完整证据见
...
| SquareSumV1 | S02BG 已验证；S02BF 待结果 | S02BG 1295/1295 + 27/27 A/B | S02BF、S02BG 分别打包审计 | S02BB 未晋级；S02BF/S02BG 待测，基线仍为 S02BA 3985.33 |
...
| --- | ---: | ---: | ---: | ---: |
| SquareSumV1 | 3985.330 | 1268.598 | 3.14× | 68.17% |
...
该包尚未获得官方隐藏 Case 结果，不覆盖冻结的 `submission-src/`，也不
宣称已达到前十或 `0.30 × T_baseline`。收到五个 Case 后逐项登记。
...
S02BF 已上传时先等待其结果；尚未上传且提交次数紧张时可直接使用
S02BG。两者均保留，互不覆盖。
...
SquareSumV1-S02BG 的候选源码、云端构建与安装产物、本地待测 ZIP、
SHA 和 1295 项独立门禁已形成可复现闭环。S02BF→S02BG 的相邻安装
27/27 A/B 表明，FP32 中间连续归约宽度 8 的目标布局取得
`1.005x～11.960x` 加速，控制布局最大绝对波动 `1.149%`。

当前不能确认的是：S02BF、S02BG 的平台 `prof_sum`、各题相对头部队伍
的最新差距以及隐藏 Case 的具体 shape/dtype/TilingKey。只有官方五项
结果可以证明本轮收益是否转化并判断是否达到 `0.30 × T_baseline`；
优化必须依据通用算法和实测 profiling，不得把平台 Case 当作可猜测或
可硬编码的数据。

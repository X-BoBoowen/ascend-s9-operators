...
>
> 当前阶段：SquareSumV1-S02CN 正式结果为 `4010.864 μs`，相对正式最好
> S02CA 的 `3982.569 μs` 回退 `0.7105%`，已淘汰。评分口径复核后建立两
> 个相互独立的本地候选：S02CQ 删除 BF16 平方结果的中间量化往返；S02CR
> 为 fastPath4 通用低并行度区域增加 40 核、二维批量搬运的 Split-K；S02CS
> 将已有 fastPath3 Split-K 补到此前遗漏的短连续尾段 `1..1023`。
> S02CR 路由契约 `12/12`、坐标与数值模型 `45/45` 通过；S02CS 路由契约
> `11/11`、数值模型 `15/15` 通过，但这些候选均尚未在
> 910B/CANN 8.5.0 构建或运行，禁止提交。正式基线为 S02CA
> `3982.569 μs`，总目标仍为 `≤1195.599 μs`。当前结构候选证据见
> [`SQUARESUM_S02CR_LOCAL_AUDIT_20260806.md`](./SQUARESUM_S02CR_LOCAL_AUDIT_20260806.md)
> 与 [`SQUARESUM_S02CS_LOCAL_AUDIT_20260806.md`](./SQUARESUM_S02CS_LOCAL_AUDIT_20260806.md)。
...
| SquareSumV1 | S02CQ/S02CR/S02CS 独立本地候选，待云端门禁 | CQ 模型 27/27；CR 路由 12/12 + 模型 45/45；CS 路由 11/11 + 模型 15/15；真机未运行 | 禁止打包 | S02CN 4010.864 已淘汰；正式最好 S02CA 3982.569 |
...
### 12.25 SquareSumV1-S02CQ/S02CR/S02CS 本地候选
...
- S02CS：把 S02CA 已经真机验证过的 fastPath3 `reduceMode=3` 补到此前
  被 `tail>=1024` 门限遗漏的短尾 `1..1023`。

S02CS 只修改一份 Host 路由文件，Kernel 与正式 S02CA 字节一致。本地公开
维度矩阵 `28/28`、路由/边界 `11/11`、三 dtype 数值模型 `15/15` 通过。
它仍未在 910B/CANN 8.5.0 构建或计时，详细边界和云端 A/B/A 门禁见
`SQUARESUM_S02CS_LOCAL_AUDIT_20260806.md`。三个候选均禁止直接打包提交。
...
并且部分 shape 超出题面维度上限。S02CQ、S02CR 与 S02CS 是复核后建立的
三个独立本地候选；它们分别处理 BF16 中间量化、fastPath4 通用低并行度，
以及 fastPath3 在短连续尾段 `1..1023` 未启用已有 Split-K 的覆盖缺口。
三者均尚无 910B 构建、正确性或性能结果，禁止打包提交。
...
不超过 3%；S02CR/S02CS 的目标矩阵总改善门槛为 10%，S02CQ 为 5%。

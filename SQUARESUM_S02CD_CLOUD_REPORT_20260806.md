...
正式平台结果已经返回，五项均 Pass，但总耗时为 `3994.349 us`，没有
达到 `0.30 × T_baseline = 1195.599 us`。这也证明公开图谱的大幅改善
没有命中正式负载对应的主要执行路径。
...
S02CD 相对 S02BA 慢 `9.019 us（0.226%）`，属于无效变化。只有后续候选
五项全部 Pass 且稳定中位总和不超过 `1195.599 us`，才能判定目标完成。

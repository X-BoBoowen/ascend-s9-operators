...
constexpr uint32_t FLOAT_ELEMENTS_PER_BLOCK =
    32U / sizeof(float);
constexpr uint32_t MAX_CONTIGUOUS_FULL_INNER = 256U;
...
private:
    ...
    __aicore__ inline void ProcessStridedInnerBulk()
    {
        ...
        while (processed < outputs_) {
            ...
            const uint32_t floatPadded =
                (current + 7U) / 8U * 8U;
            const bool contiguousFullInner =
                innerIndex == 0U &&
                current64 == innerElements_ &&
                current <= MAX_CONTIGUOUS_FULL_INNER &&
                current % elementsPerBlock != 0U;
            const uint32_t rowStride =
                contiguousFullInner ? current : paddedInner;
            const uint32_t vectorCount =
                contiguousFullInner ? current : floatPadded;
            uint32_t reduceRowsPerTile =
                CHUNK / rowStride;
            if (reduceRowsPerTile == 0U) {
                reduceRowsPerTile = 1U;
            ...
            for (uint64_t group = 0;
                ...
                while (rowOffset < lastReduceDim) {
                    ...
                    padParams.isPad =
                        contiguousFullInner
                            ? inputCount % elementsPerBlock != 0U
                            : paddedInner != current;
                    padParams.leftPadding = 0;
                    padParams.rightPadding =
                        static_cast<uint8_t>(
    ...
    __aicore__ inline void ReduceRowsIntoEight(
        AscendC::LocalTensor<float> values,
        AscendC::LocalTensor<float> accumulate,
        const uint32_t rows,
        const uint32_t rowStride,
        const bool initialize)
    {
        ...
        if (remainingRows != 0U) {
            AscendC::LocalTensor<float> tail =
                values[firstRow * rowStride];
            const uint32_t tailElements =
                remainingRows * rowStride;
            if (firstPartial) {
                AscendC::Duplicate(
                    accumulate,
                    0.0f,
    ...
    __aicore__ inline void ProcessMiddleFullRows()
    {
        AscendC::LocalTensor<T> inputLocal =
            inputBuffer_.Get<T>();
        AscendC::LocalTensor<T> outputLocal =
        ...
        for (uint64_t task = 0U; task < outputs_; ++task) {
            const uint64_t outputRow = firstOutput_ + task;
            uint64_t reduceOffset = 0U;
                ...
                const uint32_t currentReduceRows =
                    static_cast<uint32_t>(
                        reduceElements_ - reduceOffset <
                            ...
                            : reduceRowsPerTile);
                const uint32_t inputCount =
                    currentReduceRows * rowElements;
                const uint32_t paddedInputCount =
                    (inputCount + elementsPerBlock - 1U) /
                    elementsPerBlock * elementsPerBlock;
                const uint64_t inputStart =
                    (outputRow * reduceElements_ + reduceOffset) *
                    innerElements_;
            ...
            AscendC::DataCopyExtParams outputCopy;
            outputCopy.blockCount = 1U;
            outputCopy.blockLen = rowElements * sizeof(T);
            outputCopy.srcStride = 0U;
            outputCopy.dstStride = 0U;
            AscendC::DataCopyPad(
                outputGm_[outputRow * innerElements_],
                outputLocal,
                outputCopy);
            AscendC::SetFlag<AscendC::HardEvent::MTE3_S>(
                mte3ToSEvent_);

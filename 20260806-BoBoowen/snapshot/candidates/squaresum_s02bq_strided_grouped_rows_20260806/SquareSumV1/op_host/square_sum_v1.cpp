...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    const uint64_t elementsPerBlock =
        32U / inputTypeBytes;
    constexpr uint64_t STRIDED_GROUPED_WIDTH = 8U;
    ...
    uint64_t stridedGroupedPaddedRowElements = 0U;
    ...
    if (fastPath == 4U &&
        ...
        if (powerOfTwoReduce &&
            ...
            for (int32_t axis =
                 ...
                 axis >= 0;
                 --axis) {
                if (outputDims[axis] >= STRIDED_GROUPED_WIDTH &&
        ...
        if (stridedGroupedOutputAxis < outputRank) {
            ...
            stridedGroupedPaddedRowElements =
                (stridedGroupedRowElements +
                 elementsPerBlock - 1U) /

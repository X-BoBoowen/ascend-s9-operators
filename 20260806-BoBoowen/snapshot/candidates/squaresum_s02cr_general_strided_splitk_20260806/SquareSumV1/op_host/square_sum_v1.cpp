...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    constexpr uint64_t GENERAL_STRIDED_SPLITK_MIN_REDUCE = 2048U;
    constexpr uint64_t GENERAL_STRIDED_SPLITK_MAX_OUTPUTS = 8192U;
    constexpr uint64_t GENERAL_STRIDED_OUTPUT_CHUNK = 1024U;
    ...
    uint64_t generalStridedOutputChunks = 0U;
    uint64_t generalStridedBaselineCost = 0U;
    uint64_t generalStridedSplitKCost = 0U;
    bool generalStridedSplitK = false;
    if (fastPath == 4U &&
        reduceMode == 0U &&
        !stridedGroupedRows &&
        inputElements >= STRIDED_SPLITK_MIN_INPUT &&
        reduceElements >= GENERAL_STRIDED_SPLITK_MIN_REDUCE &&
        outputElements > 0U &&
        outputElements <= GENERAL_STRIDED_SPLITK_MAX_OUTPUTS &&
        innerElements > 0U &&
        ...
        const uint64_t chunksPerRow =
            (innerElements +
             GENERAL_STRIDED_OUTPUT_CHUNK - 1U) /
            GENERAL_STRIDED_OUTPUT_CHUNK;
        const uint64_t existingBlocks =
            innerElements <= 16U
                ? (outputElements < MAX_BLOCK_DIM
                    ? outputElements
                    : MAX_BLOCK_DIM)
                : (((outputElements +
                     OUTPUTS_PER_CORE_TARGET - 1U) /
                    OUTPUTS_PER_CORE_TARGET) < MAX_BLOCK_DIM
                    ? (outputElements +
                       OUTPUTS_PER_CORE_TARGET - 1U) /
                        OUTPUTS_PER_CORE_TARGET
                    : MAX_BLOCK_DIM);
        uint64_t baselineChunkWaves = 0U;
        uint64_t splitKReduceWaves = 0U;
        uint64_t doubledSplitKCost = 0U;
        if (existingBlocks > 0U &&
            SafeMultiply(
                outputRows,
                chunksPerRow,
                generalStridedOutputChunks)) {
            baselineChunkWaves =
                (generalStridedOutputChunks +
                 existingBlocks - 1U) /
                existingBlocks;
            splitKReduceWaves =
                (reduceElements + MAX_BLOCK_DIM - 1U) /
                MAX_BLOCK_DIM;
            generalStridedSplitK =
                SafeMultiply(
                    baselineChunkWaves,
                    reduceElements,
                    generalStridedBaselineCost) &&
                SafeMultiply(
                    generalStridedOutputChunks,
                    splitKReduceWaves,
                    generalStridedSplitKCost) &&
                SafeMultiply(
                    generalStridedSplitKCost,
                    2U,
                    doubledSplitKCost) &&

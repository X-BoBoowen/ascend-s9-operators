...
candidate_source="${repo_root}/candidates/squaresum_s02cq_bf16_fused_accum_20260806/SquareSumV1"
...
value = {
    ...
    "message": "S02CQ cloud gate stopped before performance comparison",
}
...
echo "S02CQ_GATE_START repo=${repo_root} template=${template_project} work=${work_root}"
cd -- "${repo_root}"

gate_stage="local_preflight"
python3 diagnostics/validate_squaresum_profile_matrix_20260806.py
python3 diagnostics/squaresum_s02cq_bf16_static_20260806.py
...
candidate_package="$(find_package "${work_root}/projects/s02cq")"
gate_stage="isolated_install"
install_package "${baseline_package}" "${work_root}/install/s02ca"
install_package "${candidate_package}" "${work_root}/install/s02cq"
baseline_env="$(find_env_file "${work_root}/install/s02ca")"
candidate_env="$(find_env_file "${work_root}/install/s02cq")"
...
run_with_opp "${candidate_env}" python3 -c \
    'import square_sum_v1_validation_lib; print("S02CQ_IMPORT_OK")'

# Official-compatible timing: 30 interleaved aclnnMul + SquareSumV1 calls,
# filter Mul rows, then take the median of task durations 10..30.
gate_stage="official_aba_profile"
run_with_opp "${baseline_env}" bash \
    diagnostics/run_squaresum_official_matrix_20260806.sh \
    bf16 S02CA_A SquareSumV1/baseline_a core

# Candidate correctness and topology discovery remain entirely inside the
# published dimensional domain.  The atlas uses the official tolerance rule.
gate_stage="candidate_correctness"
run_with_opp "${candidate_env}" python3 \
    diagnostics/squaresum_domain_event_atlas_20260806.py \
    --label S02CQ --tier atlas --dtypes fp16 bf16 fp32 \
    | tee "${detail_root}/candidate_domain_atlas.log"

gate_stage="official_aba_profile"
run_with_opp "${candidate_env}" bash \
    diagnostics/run_squaresum_official_matrix_20260806.sh \
    bf16 S02CQ SquareSumV1/candidate core
run_with_opp "${baseline_env}" bash \
    diagnostics/run_squaresum_official_matrix_20260806.sh \
    bf16 S02CA_B SquareSumV1/baseline_b core
...
if [[ ${gate_status} -ne 0 ]]; then
    echo "S02CQ_GATE_REJECT status=${gate_status} result=${artifact_dir}/result.json"
    exit "${gate_status}"
fi

echo "S02CQ_GATE_PASS result=${artifact_dir}/result.json log=${artifact_dir}/run.log"

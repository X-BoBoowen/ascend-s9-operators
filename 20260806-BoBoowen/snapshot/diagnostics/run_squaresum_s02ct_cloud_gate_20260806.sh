...
candidate_source="${repo_root}/candidates/squaresum_s02ct_last_output16_splitk_20260806/SquareSumV1"
...
value = {
    ...
    "message": "S02CT cloud gate stopped before performance comparison",
}
...
echo "S02CT_GATE_START repo=${repo_root} template=${template_project} work=${work_root}"
cd -- "${repo_root}"

gate_stage="local_preflight"
python3 diagnostics/validate_squaresum_profile_matrix_20260806.py
python3 diagnostics/squaresum_s02ct_last_output16_static_20260806.py
...
candidate_package="$(find_package "${work_root}/projects/s02ct")"
gate_stage="isolated_install"
install_package "${baseline_package}" "${work_root}/install/s02ca"
install_package "${candidate_package}" "${work_root}/install/s02ct"
baseline_env="$(find_env_file "${work_root}/install/s02ca")"
candidate_env="$(find_env_file "${work_root}/install/s02ct")"
...
run_with_opp "${candidate_env}" python3 -c \
    'import square_sum_v1_validation_lib; print("S02CT_IMPORT_OK")'
...
run_with_opp "${baseline_env}" python3 \
    ...
    --label S02CA --tier last_output_splitk --dtypes fp16 bf16 fp32 \
    | tee "${detail_root}/baseline_last_output_splitk_atlas.log"

gate_stage="candidate_correctness_and_discovery"
run_with_opp "${candidate_env}" python3 \
    diagnostics/squaresum_s02ct_correctness_20260806.py S02CT \
...
run_with_opp "${baseline_env}" bash \
    ...
    S02CA_A SquareSumV1/baseline_a last_output_splitk fp16 bf16 fp32
run_with_opp "${candidate_env}" bash \
    diagnostics/run_squaresum_official_multidtype_matrix_20260806.sh \
    S02CT SquareSumV1/candidate last_output_splitk fp16 bf16 fp32
run_with_opp "${baseline_env}" bash \
    diagnostics/run_squaresum_official_multidtype_matrix_20260806.sh \
    S02CA_B SquareSumV1/baseline_b last_output_splitk fp16 bf16 fp32
...
if [[ ${gate_status} -ne 0 ]]; then
    echo "S02CT_GATE_REJECT status=${gate_status} result=${artifact_dir}/result.json"
    exit "${gate_status}"
fi

echo "S02CT_GATE_PASS result=${artifact_dir}/result.json log=${artifact_dir}/run.log"

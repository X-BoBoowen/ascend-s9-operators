...
## 7. 官方结果与结论（2026-08-06）

```text
Case1: Pass, Result: 6.470
Case2: Pass, Result: 390.5975
Case3: Pass, Result: 139.463
Case4: Pass, Result: 2552.781
Case5: Pass, Result: 893.4775
prof_sum: 3982.789
```

相对 S02BA `3985.330 us`，S02BI 合计只改善 `2.541 us（0.064%）`；
五项变化为 `-0.040 / -8.4905 / +0.710 / +2.020 / +3.2595 us`。
Case4/5 反而退化，合计差异也远小于既有同包复测波动，因此不能把它视为
有效性能提升。S02BI 判定不晋级，正式优化基线继续使用 S02BA。

这份结果同时提供了负证据：虽然 fastPath3 split-K 在公开通用矩阵取得
显著提升，但没有命中官方隐藏主耗时布局。后续继续依据通用布局和真机
profiling 优化未覆盖路径，不反推或硬编码隐藏 shape。

...
static ge::graphStatus TilingFunc(gert::TilingContext* context)
{
    ...
    bool stridedGroupedPaddedRows = false;
    uint64_t innerWithPadding = 0U;
    uint64_t alignedInnerElements = 0U;
    const bool alignedInnerOk =
        SafeAdd(
            innerElements,
            elementsPerBlock - 1U,
            innerWithPadding) &&
        SafeMultiply(
            innerWithPadding / elementsPerBlock,
            elementsPerBlock,
    ...
    if (fastPath == 4U &&
        ...
        const bool stridedGroupedNarrowType =
            inputTypeBytes == sizeof(uint16_t);
        const bool stridedGroupedSmallInner =
            stridedGroupedNarrowType &&
            innerElements % 8U != 0U &&
            innerElements <= 16U &&
            lastReduceDim > 1U &&
            ...
            if (groupedOutputAxis < outputRank) {
                ...
                if (SafeMultiply(
                    ...
                    for (uint64_t width =
                        ...
                        uint64_t alignedElementsPerTask = 0U;
                        ...
                        stridedGroupedPaddedRows = paddedRows;
                        stridedGroupedRows = !paddedRows;
                        break;
                    }
                }
            }
    ...
    if (stridedGroupedPaddedRows || stridedGroupedRows) {
        desiredBlocks =
    ...
    const uint64_t scheduledOutputs =
        (stridedGroupedPaddedRows || stridedGroupedRows)
            ? stridedGroupedTasks
            : (groupedVector8
                ? outputElements / 8U
                : outputElements);

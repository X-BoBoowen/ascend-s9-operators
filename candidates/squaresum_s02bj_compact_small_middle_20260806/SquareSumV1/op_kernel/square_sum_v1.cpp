...
private:
        ...
        const uint64_t reduceCount)
    {
        ...
        while (outputIndex < outputElements_) {
            ...
            const uint32_t elementsPerBlock =
                32U / sizeof(T);
            const bool compactFullSmallInner =
                innerElements_ > 0U &&
                innerElements_ <= 8U &&
                innerIndex == 0U &&
                current == innerElements_;
            ...
            while (reduceOffset < reduceCount) {
                ...
                const uint32_t compactCopyPadded =
                    compactFullSmallInner
                        ? (inputCount + elementsPerBlock - 1U) /
                            elementsPerBlock * elementsPerBlock
                        : inputCount;
                const uint32_t vectorInputCount =
                    compactFullSmallInner
                        ? compactCopyPadded
                        : inputCount;
                ...
                AscendC::DataCopyExtParams copyParams;
                copyParams.blockCount = compactFullSmallInner
                    ? 1U
                    : static_cast<uint16_t>(currentReduceRows);
                copyParams.blockLen = compactFullSmallInner
                    ? inputCount * sizeof(T)
                    : current * sizeof(T);
                copyParams.srcStride = compactFullSmallInner
                    ? 0U
                    : static_cast<uint32_t>(
                        (innerElements_ - current) * sizeof(T));
                copyParams.dstStride = 0;
                AscendC::DataCopyPadExtParams<T> padParams;
                padParams.isPad = compactFullSmallInner
